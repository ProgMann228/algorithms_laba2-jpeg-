import numpy as np
"""
def generate_size_table(bits):
    huffsize=[]
    k=0
    i=1  # длина кода от 1 до 16
    j=1  # сколько кодов уже назначено текущей длины
    while i <= 16:
        if j <= bits[i - 1]:  # bits[0] — для длины 1
            huffsize.append(i)
            k += 1
            j += 1
        else:
            i += 1
            j = 1  # сброс счётчика для новой длины

    huffsize.append(0)  # по стандарту: последний ноль
    lastk = k
    return huffsize, lastk
    
def generate_code_table(huffsize):
    huffcode = []
    code = 0
    si = huffsize[0]
    for k in range(len(huffsize) - 1):
        while huffsize[k] == si:
            ...
            if k >= len(huffsize) - 1:
                break
        code <<= 1  # переход на следующую длину
        si += 1
    return huffcode
...
def generate_size_table(bits):
    huffsize = []
    for i in range(1, 17):  # длины от 1 до 16
        for _ in range(bits[i-1]):
            huffsize.append(i)
    huffsize.append(0)  # по стандарту
    return huffsize
...
def generate_code_table(huffsize):
    ...
    while True:
        ...
        code += 1
        k += 1
        if k < len(huffsize) and huffsize[k] != si:
            while k < len(huffsize) and huffsize[k] != si:
                code <<= 1
                si += 1
    return huffcode

def order_codes(huffval, huffcode, huffsize):
    EHUFCO = {}  # Код для каждого значения
    EHUFSI = {}  # Длина кода для каждого значения
    for k in range(len(huffval)):
        I = huffval[k]
        EHUFCO[I] = huffcode[k]
        EHUFSI[I] = huffsize[k]
...
# Таблица K.5 для AC
# Таблица K.5 - Luminance AC coefficients (весь диапазон)
AC_luminance_table = {
    '0/0': '1010',
    '0/1': '00',
    '0/2': '01',
